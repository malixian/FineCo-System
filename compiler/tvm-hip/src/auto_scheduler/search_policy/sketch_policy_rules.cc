...
PopulationGenerationRule::ResultKind InitThreadBind::Apply(SketchPolicyNode* policy, State* state,
  ...
  //StdCout(1) << "============== Init Thread Bind ==============" << std::endl;
  char *pathvar;
  pathvar = getenv("SET_TVM_BLOCK");
  int target_block_size = pathvar ? atoi(pathvar) : 0;
  ...
  for (int stage_id = (*state)->stages.size() - 1; stage_id >= 0; --stage_id) {
    ...
    if (HasCrossThreadReduction(*state, stage_id)) {
      ...
      *state = std::move(FuseAllOuterSpaceIterators(*state, stage_id, &fused_it));
      auto block_iter = state->bind(stage_id, fused_it, IteratorAnnotation::kBlockX);
      int block_size = int(GetExtent(block_iter));
      state->_block_size = block_size;
      if(target_block_size > 0 && block_size != target_block_size) return ResultKind::kInvalid;
      continue;
    }
    ...
    if (stage->compute_at == ComputeAtKind::kRoot) {
      ...
      if (!multi_level_tiling_root_set.count(stage_id)) {
        ...
        *state = FuseAllOuterSpaceIterators(*state, stage_id, &fused_it);
        ...
        if (GetExtent(fused_it) <= policy->search_task->hardware_params->warp_size) {
          ...
          StdCout(1) << "======== 821 block : " << split_its[0] << " ====== thread: " << split_its[1] << std::endl;
      ...
      auto block_iter = state->bind(stage_id, blockidx_it, IteratorAnnotation::kBlockX);
      int block_size = int(GetExtent(block_iter));
      state->_block_size = block_size;
      
      // std::cout<<"=========== Target Block Size is:"<<target_block_size<<std::endl;
      if (target_block_size > 0 && block_size != target_block_size) {
        return ResultKind::kInvalid;
      }
