...
Array<State> SketchPolicyNode::SampleInitPopulation(const Array<State>& sketches) {
  ...
  while ((static_cast<int>(out_states.size()) < sample_init_min_pop_) && iter < 100) {
    ...
    for (auto tmp_s : temp_states) {
      if (tmp_s.defined()) {
        int block_size = tmp_s._block_size;
        //std::cout<<"temp block size: "<<block_size<<std::endl;
        cand_states.push_back(tmp_s);
        //std::cout<<"push back block size: "<<cand_states[cand_states.size()-1]._block_size<<std::endl;
      } else {
        fail_ct++;
      }
    }
      ...
      while (pnext->size() < population) {
        ...
        if (dis(rand_gen) < mutation_prob) {
          ...
          if (rule->Apply(this, &tmp_s, &rand_gen) == PopulationGenerationRule::ResultKind::kValid) {
            int tmp_block_size = tmp_s._block_size;
