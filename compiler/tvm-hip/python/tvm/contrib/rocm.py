...
def callback_rocm_link(obj_bin):
    ...
    # 输出并保存 rocm kernel
    save_gene = int(os.getenv("GEN_ENABLE", "0"))
    model = os.getenv("GEN_MODEL", "")
    layer = "layer" + os.getenv("GEN_LAYER", "")
    op = os.getenv("GEN_OP", "")
    candidate = "candidate" + os.getenv("GEN_SEQ", "")
    base_path = "/home/chr/repos/fineco-hip/include/kernel/rocm/" + model + "/"
    if layer != "layer":
        base_path += layer + "/"
    base_path += op + "/" + candidate
    # print(save_gene, base_path)
    if save_gene > 0:
        tmp_obj = base_path + ".o"
        tmp_cobj = base_path + ".hsaco"
    with open(tmp_obj, "wb") as out_file:
        out_file.write(bytes(obj_bin))
