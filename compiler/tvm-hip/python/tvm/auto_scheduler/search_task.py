...
from .measure_record import load_best_record, load_records
...
class SearchTask(Object):
    ...
    def apply_by_id(self, log_file, log_id, include_compatible=False, layout_rewrite_option=None):
        """Apply the history best from a log file and return the schedule.
        ...
        """
        ret = []
        outs = load_records(log_file)
        if outs is None:
            raise RuntimeError(
                "Cannot find any valid schedules" 
            )
    ...
    def filter_valid(self, log_file):
        idx = 0
        res_list = {}
        res_all = []
        valid_idx_list = []
        ...
        for out in outs:
            idx += 1 # 对应于1000次调优里面的原始 id
            res = out[1]
            if res.error_no != 0:
                continue
            valid_idx_list.append(idx)
        print(valid_idx_list)
        print("valid idx len:", len(valid_idx_list))

    
    def filter_effective(self, log_file, include_compatible=False):
        idx = 0
        effective_list = []
        all_list = []
        idx_cost = {}
        ...
        for out in outs:
            ...
            if self.workload_key is not None:
                dis_f = calc_workload_dis_factor(
                    decode_workload_key(self.workload_key), decode_workload_key(inp.task.workload_key)
                )
                if dis_f == float("inf"):
                    continue
        ...
        for idx in all_list:
            if idx_cost[idx] < best_cost * 2:
                effective_list.append(idx)
        print(effective_list)
        print(len(effective_list))
    ...
    def print_best(self, log_file, print_mode="schedule"):
        ...
        if print_mode == "cuda" or print_mode == "rocm":
            assert self.target.kind.name == print_mode              # print_mode 为 rocm 输出 llvm ir
    ...
    def save_spec_source(self, log_file, idx, save_file):
        """Print the best schedule as python schedule API code or CUDA source code.
        ...
        for out in outs:
            ...
            if cur_idx == idx:
                ...
                save_path = save_file + ".cuh"
                with open(save_path, "w+") as sp:
                    new_source = source.replace("default_function_kernel0", save_file, 1)
                    sp.write(new_source)
                break


    def generate_code_base(self, log_file, valid_idx_file, kernel_name):
        valid_idx_str = ""
        with open(valid_idx_file, "r") as vr:
            valid_idx_str = vr.readline().replace("[", "").replace("]", "").replace(" ", "").replace("\n", "")
        valid_idx_list = valid_idx_str.split(",")
        ...
        for out in outs:
            ...
            for valid_idx in valid_idx_list:
                if cur_idx == int(valid_idx):
                    print("match valid idx", cur_idx)
                    inp = out[0]
                    sch, args = self.compute_dag.apply_steps_from_state(inp.state)
                    func = build(sch, args, "cuda")
                    source = func.imported_modules[0].get_source()
                    base_path = "/home/malixian/repos/tvm-test/ai-bench/code_base_alex_layer1_100/"
                    save_path = base_path + kernel_name + "_" + str(cur_idx) + ".cuh"
                    with open(save_path, "w+") as sp:
                        new_source = source.replace("default_function_kernel0", kernel_name + "_" + str(cur_idx), 1)
                        sp.write(new_source)
                    break
        print("========== generate all code successful ============")
