...
class TestQuadModel : public Test {
 public:

    TestQuadModel(const string& task_1_name, const string& task_2_name, const string& task_3_name, const string& task_4_name) : _task_1_name(task_1_name),\
     _task_2_name(task_2_name), _task_3_name(task_3_name),_task_4_name(task_4_name)  {}

    template<typename M1, typename M2, typename M3, typename M4>
    void ServeBench(int request_num_1, int request_num_2, int request_num_3, int request_num_4,\
    int sampling_num, int stream_num) {
        ...
        shared_ptr<M4> _model_4(new M4(_task_4_name));

        _model_1->InitModel(ROCM);
        _model_2->InitModel(ROCM);
        _model_3->InitModel(ROCM);
        _model_4->InitModel(ROCM);
        ...
        vector<int> task_4_rs;
        GenRequestStream(request_num_4, task_4_rs);
        ...
        vector<int> task_4_rl;
        GenRandomList(GetRequestSize(task_4_rs), task_4_rl);

        vector<int> task_1_candidate_cnt;
        vector<int> task_2_candidate_cnt;
        vector<int> task_3_candidate_cnt;
        vector<int> task_4_candidate_cnt;
        ...
        _model_4->GetAllLayerCandidateCnt(task_4_candidate_cnt);

        int all_request_num = GetRequestSize(task_1_rs) + GetRequestSize(task_2_rs)\
         +  GetRequestSize(task_3_rs) +  GetRequestSize(task_4_rs);
        ...
        for (int i=0; i<100; i++) {
            ...
            GenDefaultCombination(task_4_candidate_cnt, task_4_random_comb);
            ...
            future<float> task_4_latency = async(&Model::Run, _model_4, std::ref(task_4_random_comb), std::ref(task_4_rs),\
            _model_4->GetModelName(), task_4_stream_num, std::ref(task_4_rl), std::ref(dnn_4_run_ret),false);


            float task_1_ret = task_1_latency.get();
            float task_2_ret = task_2_latency.get();
            float task_3_ret = task_3_latency.get();
            float task_4_ret = task_4_latency.get();
        }
        ...
        for (int i=0; i<sampling_num; i++) {
            ...
            for(int repeat=0; repeat<repeat_num; repeat++) {
                ...
                if(sampling_num > 1 && i==0) {
                    ...
                    GenRandomCombination(task_4_candidate_cnt, task_4_random_comb);
                ...
                task_4_ret_list[i].push_back(task_4_ret);
                ...
                float throughput = dnn_1_run_ret[0] + dnn_2_run_ret[0] + dnn_3_run_ret[0] + dnn_4_run_ret[0];
        ...
        for(auto& item : task_4_ret_list) {
            ...
            avg_lat /= repeat_num;
            //cout<<"Resnet "<<item.first<<" avg latency:"<<avg_lat;
            task_4_avg_lat_list.push_back(avg_lat);
        }
        ...
        float default_task_4_lat = task_4_avg_lat_list[0];
        ...
        for(auto& item : co_tp_list) {
            ...
            float lat_4 = task_4_avg_lat_list[comb_id];
            if (avg_tp > best_tp) {
                best_tp = avg_tp;
                best_tp_id = comb_id;
            }
        ...
        for(int i=0; i<task_1_avg_lat_list.size(); i++) {
            ...
            float lat4 = task_4_avg_lat_list[i];
            if (min_latency > lat1 + lat2 + lat3 + lat4) {
                min_lat_idx = i;
                min_latency = lat1 + lat2 + lat3 + lat4;
            }
        }
        ...
        <<" "<<_model_3->GetModelName()<<" avg latence:"<<task_3_avg_lat_list[best_tp_id]\
        <<" "<<_model_4->GetModelName()<<" avg latence:"<<task_4_avg_lat_list[best_tp_id]<<endl;
        ...
        <<" Best Latency 3: "<<task_3_avg_lat_list[min_lat_idx]\
        <<" Best Latency 4: "<<task_4_avg_lat_list[min_lat_idx]<<endl;
        ...
        <<" "<<_model_3->GetModelName()<<" avg latence:"<<default_task_3_lat\
        <<" "<<_model_4->GetModelName()<<" avg latence:"<<default_task_4_lat<<endl;
        //delete _model_1;
        //delete _model_2;
    }
    ...
    private:
        ...
        unordered_map<int, vector<float>> task_4_ret_list;
        ...
        vector<float> task_4_avg_lat_list;
