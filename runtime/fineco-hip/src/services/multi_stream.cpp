...
class MultiStreamService : public PairService {
 public: 

    void Accept(int set_stream_num=2) {
        cout<<"==== Start Multi-Stream Service ========"<<endl;
        ...
        const char *maskEnv[2][4] = {{"CUMASK0_0", "CUMASK0_1", "CUMASK0_2", "CUMASK0_3"},
                                     {"CUMASK1_0", "CUMASK1_1", "CUMASK1_2", "CUMASK1_3"}
                                     };
        ...
        for(int i=0; i<stream_num; i++) {
            ...
            if (i < default_stream_num) {
                if (tmp && i < 2)  {
                    uint32_t cuMask[4] = {0xffffffff, 0xffffffff, 0xffffffff, 0x00ffffff};
                    for (int j = 0; j < 4; j++) {
                        tmp = getenv(maskEnv[i][j]);
                        if (tmp)
                            sscanf(tmp, "%x", &cuMask[j]);
                    }
                    cout << "enabel CU MASK" << endl;
                    _stream_pool[i] = GetBackendHandle(_model_1->GetModelRuntime())->CreateStream(4, cuMask);
                } else
        ...
        while(_total_request_queue.size() > 0) {
            ...
            for(int request_id = 0; request_id < tmp_req.size(); request_id+=stream_num) {
                ...
                int need_stream_num = (request_id + stream_num > tmp_req.size()) ? tmp_req.size() - request_id : stream_num; 
                for (int stream_id=0; stream_id < need_stream_num; stream_id++) {
                    auto cur_request = tmp_req[request_id+stream_id];
