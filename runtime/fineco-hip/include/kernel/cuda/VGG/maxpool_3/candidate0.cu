...
extern "C" __global__ void __launch_bounds__(49) candidate0(float* __restrict__ tensor, float* __restrict__ data) {
  ...
  for (int rv0 = 0; rv0 < 2; ++rv0) {
    for (int rv1 = 0; rv1 < 2; ++rv1) {
      tensor[((((int)blockIdx.x) * 49) + ((int)threadIdx.x))] = max(tensor[((((int)blockIdx.x) * 49) + ((int)threadIdx.x))], data[(((((((((int)blockIdx.x) * 7) + (((int)threadIdx.x) / 7)) >> 2) * 112) + (rv0 * 56)) + ((((((int)blockIdx.x) * 49) + ((int)threadIdx.x)) % 28) * 2)) + rv1)]);
    }
  }
}
