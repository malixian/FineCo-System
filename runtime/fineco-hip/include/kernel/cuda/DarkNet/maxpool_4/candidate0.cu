...
extern "C" __global__ void __launch_bounds__(8) candidate0(float* __restrict__ tensor, float* __restrict__ data) {
  ...
  for (int rv0 = 0; rv0 < 2; ++rv0) {
    for (int rv1 = 0; rv1 < 2; ++rv1) {
      tensor[((((int)blockIdx.x) * 8) + ((int)threadIdx.x))] = max(tensor[((((int)blockIdx.x) * 8) + ((int)threadIdx.x))], data[(((((((((int)blockIdx.x) * 4) + (((int)threadIdx.x) >> 1)) / 7) * 56) + (rv0 * 28)) + ((((((int)blockIdx.x) * 8) + ((int)threadIdx.x)) % 14) * 2)) + rv1)]);
    }
  }
}
