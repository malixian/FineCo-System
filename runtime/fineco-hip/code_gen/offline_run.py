...
def gene_kernel_by_name_resnet(sub_model_name, layer_list):
    for layer_idx in range(1, len(layer_list)+1):
        ...
        for kernel_idx in range(1, len(layer)+1):
            ...
            gene_dir =  "../include/kernel/rocm/" + sub_model_name + "/" + "layer" + str(layer_idx) + "/" + kernel_name
            os.environ["GEN_ENABLE"] = "1"
            os.environ["GEN_MODEL"] = sub_model_name
            os.environ["GEN_LAYER"] = str(layer_idx)
            os.environ["GEN_OP"] = kernel_name
...
def generate_candidate_kernel(sub_model_name):
    ...
    if sub_model_name == "mobilenet":
        ...
        kernels = [
            # "gene_w_qkv", "add",
            "gene_qkv", "compute_qk", "compute_sv", 
            # "multi_head_concat_1", "multi_head_concat_2", "multi_head_concat_3", "multi_head_concat_4", 
...
def gene_kernel_by_name(kernel_list, sub_model_name):
    ...
    for kernel in kernel_list:
        gene_dir = "mkdir -p " + " ../include/kernel/rocm/" + model_file_name + "/" +  kernel + "/"
        os.environ["GEN_ENABLE"] = "1"
        os.environ["GEN_MODEL"] = model_file_name
        # os.environ["GEN_LAYER"] = 
        os.environ["GEN_OP"] = kernel
        result = os.popen(gene_dir) 
        # res = result.read()
        ...
        btf = open("../include/kernel/rocm/" + model_file_name + "/" +  kernel + "/" + "candidate_params.txt", "a+")
        print("../include/kernel/rocm/" + model_file_name + "/" +  kernel + "/" + "candidate_params.txt")
        btf.write("Block Thread" + "\n")

        cnt = 0
        for i in range(0, len(grid_size_list)):
            # kf = open("../include/kernel/rocm/" + model_file_name + "/" +  kernel +  "/" + "candidate" + str(i) + ".cu", "w")
...
def applay_candidate_kernel(sub_model_name):
    ...
    if sub_model_name == "mobilenet":
        ...
        kernels = [
            "conv2d_block1_1", "conv2d_block1_2", "conv2d_block2_1","conv2d_block2_2",\
            "conv2d_block3_1","conv2d_block3_2","conv2d_block4_1","conv2d_block4_2",\
            "conv2d_block5_1",
    ...
    for kernels in layer_list:
        for task_idx in range(0, len(kernels)):
            if "conv" in kernels[task_idx]:
                ...
                gene_dir = "mkdir -p " + " ../include/kernel/rocm/" + sub_model_name + "/" +  layer_idx_str + "/" +  conv_name_str + "/"
                result = os.popen(gene_dir) 
                res = result.read() 
                print(gene_dir)    
                for i in range(0, len(layer_mps[conv_name_str])):
                    btf = open("../include/kernel/rocm/" + sub_model_name + "/" +  layer_idx_str + "/" +  conv_name_str + "/" + "candidate_params.txt", "a+")
                    kf = open("../include/kernel/rocm/" + sub_model_name + "/" +  layer_idx_str + "/" +  conv_name_str + "/" + "candidate" + str(i) + ".cu", "w")
                ...
                gene_dir = "mkdir -p " + " ../include/kernel/rocm/" + sub_model_name + "/" +  kernel_str + "/"
                result = os.popen(gene_dir)
                has_conv = False
                for i in range(0, len(layer_mps[kernel_str])):
                    btf = open("../include/kernel/rocm/" + sub_model_name + "/" + kernel_str + "/" + "candidate_params.txt", "a+")
                    kf = open("../include/kernel/rocm/" + sub_model_name + "/" + kernel_str + "/" + "candidate" + str(i) + ".cu", "w")
...
if __name__ == "__main__":
    ...
    # generate_resnet_candidate_kernel()
    # generate_candidate_kernel("vgg")
    # generate_candidate_kernel("darknet")
    # generate_candidate_kernel("mobilenet")
    # generate_candidate_kernel("bert")
