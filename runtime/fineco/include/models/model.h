...
#include "../src/backend/cuda/cuda_timer.cc"
...
class  Model {
 public:
  ...
  string GetCandidatePath(const string& model_name, \
            int layer_idx, RUNTIME runtime = CUDA) {
    if(runtime != CUDA) {
      return "";
    }
    string runtime_str = "cuda";
    string base_path = "./include/kernel/" + runtime_str + "/";
  ...
  int GetCandidateCount(const string& model_name, int layer_idx, RUNTIME runtime = CUDA) {
  ...
  void ConfigureLayerCandidateByPath(Layer* layer, const string& sub_model_name,  const string& layer_name) {
    ...
    string base_path = "./include/kernel/";
    if (_runtime == CUDA) {
        base_path += "cuda/";
    }
  ...
  virtual float RealRun(vector<int>& selector, float* output, float* input, size_t input_size, DLStream stream, bool return_latency=false) {
    ...
    shared_ptr<CUDATimer> timer;
    if(return_latency) {
      timer = make_shared<CUDATimer>(stream);
      timer->Start();
    }
  ...
  virtual float RunWithMultiStream(vector<int>& selector, vector<int>& requests, const string& task_name, int stream_num, vector<int>& random_list, vector<float>& ret_list, bool print_latency) {
    ...
    CUDATimer timer(_run_stream_list[0]);
    timer.Start();
