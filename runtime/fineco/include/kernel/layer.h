...
class Layer {
 public:
  ...
  Layer(RUNTIME runtime=CUDA) : _runtime(runtime){}
  ...
  float* GenGPUData(size_t size) {
    ...
    auto d = static_cast<float*>(device_api->AllocDataSpace(mem_size, DLFLOAT));
  ...
  void RegisterCandidate(const string& model_name, const string& layer_idx_str, \
    ...
    if (_runtime == CUDA) {
      auto function = static_cast<CUfunction>(device_api->GetFunction(model_name, layer_idx_str, candidate_name, layer_name));
 ...
 public:
  ...
  unordered_map<string, CUfunction> _function_list;
