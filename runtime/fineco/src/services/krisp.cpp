...
class KrispService : public PairService {
 public: 

    void Accept(int set_stream_num=2) {
        cout<<"==== Start Krisp Service ========"<<endl;
        ...
        this->GetBestEffectImplList(1.5, _model_name_1, candidate_high_selector_1, test_stream);
        vector<int> candidate_low_selector_1 = candidate_high_selector_1;
        
         vector<int> candidate_cnt_2;
        _model_2->GetAllLayerCandidateCnt(candidate_cnt_2);
        vector<int> candidate_high_selector_2;
        this->GetBestEffectImplList(1.5, _model_name_2, candidate_high_selector_2, test_stream);
        vector<int> candidate_low_selector_2 = candidate_high_selector_2;
        ...
        while(_total_request_queue.size() > 0) {
            ...
            for(int ri=0; ri<_total_request_queue.size() && _total_request_queue[ri]->GetGeneTimestep() == first_timestamp; ri++) {
                if(_total_request_queue[ri]->GetQoSKind() == HighQoS) {
                    tmp_high_req.push_back(_total_request_queue[ri]);
                } else if (_total_request_queue[ri]->GetQoSKind() == LowQoS) {
                    tmp_low_req.push_back(_total_request_queue[ri]);
                }
            }


            auto arrival_timestamp = chrono::steady_clock::now();
            for(int request_id = 0; request_id < tmp_high_req.size(); request_id+=stream_num) {
                ...
                int need_stream_num = (request_id + stream_num > tmp_high_req.size()) ? tmp_high_req.size() - request_id : stream_num; 
                for (int stream_id=0; stream_id < need_stream_num; stream_id++) {
                    auto cur_request = tmp_high_req[request_id+stream_id];
            ...
            for(int request_id = 0; request_id < tmp_low_req.size(); request_id+=stream_num) {
                vector<future<float>> results_model1_rt;
                vector<future<float>> results_model1_be;
                vector<future<float>> results_model2_rt;
                vector<future<float>> results_model2_be;
                int need_stream_num = (request_id + stream_num > tmp_low_req.size()) ? tmp_low_req.size() - request_id : stream_num; 
                for (int stream_id=0; stream_id < need_stream_num; stream_id++) {
                    auto cur_request = tmp_low_req[request_id+stream_id];
                    auto stream = _stream_pool[stream_id];
                    if (cur_request->GetModelKind() == M1 && cur_request->GetQoSKind() != Empty) {
                        submit_number++;
                        if (cur_request->GetQoSKind() == HighQoS) {
